...
export async function editLibranza(req: AuthenticatedRequest, res: Response) {
    try {
        ...
        // Solo analistas y admins pueden editar
        if (adminRole !== "CREDIT_ANALYST" && adminRole !== "ADMIN") {
            return res.status(403).json({ ok: false, message: "No tienes permisos para editar libranzas" });
        }
        ...
        if (contract.status === "SIGNED") {
            return res.status(400).json({ ok: false, message: "No se puede editar una libranza firmada" });
        }

        if (contract.status === "CANCELLED") {
            return res.status(400).json({ ok: false, message: "No se puede editar una libranza anulada" });
        }
