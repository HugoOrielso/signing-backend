...
export async function downloadSignedContractById(req: Request, res: Response) {
  let browser;

  try {
    const contractId = Array.isArray(req.params.id)
      ? req.params.id[0]
      : req.params.id;
    ...
    if (!contractId) {
      return res.status(400).json({
        ok: false,
        message: "El contractId es requerido",
      });
    }
    ...
    if (!contract) {
      return res.status(404).json({
        ok: false,
        message: "El contrato no está disponible para descarga",
      });
    }
    ...
    console.error("DOWNLOAD PDF BY ID ERROR:", error);
